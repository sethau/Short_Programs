...
def nextMove(player, board):
	...
	squares = [[[28, 28], [28, 27], [27, 27], [27, 28]], [[28, 24], [28, 23], [27, 23], [27, 24]],
	[[24, 28], [23, 28], [23, 27], [24, 27]], [[24, 24], [24, 23], [23, 23], [23, 24]]]
	newBoard = []
	...
	#disrupt the pattern
	numCells = 0
	if (brSquaresClear(newBoard, 23, opp)):
	...
	while not success and mod < 100:
		...
		if mod % 3 == 0:
			if not success:
				success, x, y = assistUR(newBoard, mod, 'a')
			if not success:
				success, x, y = assistDL(newBoard, mod, 'a')
			if not success:
...
def brSquaresClear(board, bound, opp):
